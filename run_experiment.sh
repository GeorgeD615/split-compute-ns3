...
rm -f "$FILES_STORAGE"/prediction_results.csv
rm -f "$FILES_STORAGE"/energy_log.csv
rm -f "$FILES_STORAGE"/latency_log.csv
...
for DIST in "${DISTANCES[@]}"; do
    ...
    GT_LABEL=$(basename "$IMAGE_PATH" .png)
        ...
        python3 "$ML_DIR/edge_postprocess.py" "files/latency_log.csv" "files/prediction_results.csv"
    done
done
